...
class Gallery {
    ...
    function Gallery($database, $form, $usermanagement, $site) {
		...
		$this->familyCategories = $this->getFamilyCategories($this->username);
		$this->familyPhotos = $this->getFamilyPhotos($this->username);
		...
		if (isset($_POST['submit-vote'])) {
			...
			if (!empty($this->cid) && !in_array($this->cid, $this->familyCategories)) {
				header("Location: photogallery.php");
				return;
			}
			if (!empty($this->pid) && !in_array($this->pid, $this->familyPhotos)) {
				header("Location: photogallery.php");
				return;
			}
			$this->displayGallery();
   ...
   function getNumNewMessages($username) {
     ...
     * getFamilyPhotos
     * 
     * Used to get ids array of all the photo of user's family. 
     */
	function getFamilyPhotos($username) {
		 
		 $username = $this->site->cleanOutput($username);
		 
		$q = "SELECT p.photo_id
				FROM ".TBL_GALLERY_PHOTOS." AS p 
				...
				AND p.photo_user = m.manager_username";
	...
	function displayCategories($username, $cid, $page) {
		...
		$result_final .= "	<a class='add-new-icon' href='".$this->url."?add-category=0#menu'>Naujas albumas</a>
						</div>
						<table class='gallery'>";
	...
	function displayThumbnails($cid, $page) {
		...
		$result_final = "<div class='gallery-menu'>";
		$this->displaySearchForm();
		$result_final .= "	<a class='add-new-icon' href='upload.php?photo=$cid#menu'>Nauja nuotrauka</a>
							<a class='edit-icon' href='".$this->url."&edit-category=".$cid."#menu'>Redaguoti albumą</a>
		...
		if (empty($num_rows)) {
			$result_final .= "<div class='notice'><p>Albume nėra nuotraukų. Įkelkite naują <a href='upload.php?photo=$cid#menu'>nuotrauką</a>!</p></div>";
		}
		else {
			while($row = mysql_fetch_array($result)) {
				$result_array[]= "
								<div class='thumbs'>
									<a href='photogallery.php?cid=$cid&amp;pid=".$row['photo_id']."' title='".$row['photo_title']."#menu'>
			...
			$result_final = "<table class='gallery'>
								<tr class='controls'>
									<td class='nav-categories'>  
										<a href=photogallery.php#menu>Albumai</a> &raquo  
	...
	function displayComments($cid, $pid, $page) {
	   ...
	   if ((!empty($cid)) && (!empty($pid))) {
		   ...
		   $num_rows = mysql_numrows($result);
			...
			$results_final .= "<div class='comment-form'>
									<form method='post' action=''>
											<textarea name='comment' value='".$this->formError->value("comment")."'placeholder='Parašykite savo komentarą čia'></textarea>
											<br>".$this->formError->error("comment")."
											<p class='submit'>
												<input type='submit' name='add-comment' value='Komentuoti'/>
											</p>
									</form>
								</div>";
	...
	*/
	function displaySearchResults() {
		...
		$q = "SELECT photo_id, photo_extension, photo_title, photo_category, photo_user 
				FROM ".TBL_GALLERY_PHOTOS.",".TBL_GALLERY_CATEGORY.",".TBL_MANAGERS." 
				WHERE manager_family = (
					SELECT individual_family
					FROM ".TBL_INDIVIDUALS."
					WHERE individual_username = '$username' 
				...
				AND category_user = manager_username 
				AND photo_category = category_id 
		...
		if(empty($num_rows)) {
			...
			// Results count
			$q =  "SELECT COUNT(*) AS num 
					FROM ".TBL_GALLERY_PHOTOS.",".TBL_MANAGERS." 
					WHERE manager_family = (
						SELECT individual_family 
						FROM ".TBL_INDIVIDUALS." 
						WHERE individual_username = '$username'
	...
	function addCommentSubmit() {
		...
		$path = "photogallery.php?cid=$cid&pid=$pid#menu";
		$field = "comment";
    ...
    */
    function deletePhotoSubmit() {
		...
		$path = "photogallery.php?cid=$cid#menu";
   ...
   /**
    ...
    function displayConfirmDelete() {
		...
		if (isset($_GET['del-category'])) {
			...
			$path = "photogallery.php?cid=$this->cid";
			?>
			<form action="" method="post">
				<div>
